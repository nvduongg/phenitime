...
import { Card, DatePicker, Button, Space, Spin, Select, Tag, Tooltip } from 'antd'
...
function PeriodBar({ daySlots }) {
  ...
  const freeCount = daySlots.filter(Boolean).length === 0
    ? 12
    : daySlots.filter(v => !v).length
...
function EmptyRooms() {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true)
      try {
        const [ttRes, roomsRes] = await Promise.all([getTimetables(), getRooms()])
  ...
  // busyLookup[room_id][day] = boolean[12]
  ...
  return (
    <Spin spinning={loading}>
      ...
      >
        {(rooms || []).map(room => {
          const roomSlots = busyLookup[room.room_id] || {}
